...
Configure your site's branding, palette, and contact details in **Club → Settings** (see the [Club Settings guide](/docs/club-management/settings.md) for details), then manage your content in the **Website** section. Changes publish the moment you save:
...
You can *restrict* individual pages so that they can only be seen by members. You can also require a specific product or fee purchase before the page becomes visible. By default, pages are unrestricted.
...
Each article includes a title, publish date, optional image, and body content. The order in which articles appear is based on the published date (most recent first).
...
You can also choose whether it shows up only on the Pages and Articles sections or across the entire site. We recommend against displaying it everywhere because it takes up a significant amount of screen space.
...
## DNS Redirect[​](#dns-redirect "Direct link to DNS Redirect")
...
* Open `Club → Support` in Curling IO and submit a support request. The [Support guide](/docs/club-management/support.md) has the full walkthrough if you need a refresher.
