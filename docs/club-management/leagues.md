...
It's required for the system to apply seasonal membership and affiliate fees, and multiple item discounts.
...
Also if Sue's club was running a multi-item discount like 20% off the second item, then the system needs to know how many items (leagues, programs, fees, products) she has already purchased in order to apply the discount appropriately.
...
If you've enabled accounting in your [Club Settings](/docs/club-management/settings#accounting), you'll see an **Accounting Codes** section when creating or editing a league or competition.
